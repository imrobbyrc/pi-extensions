...
export function buildLeadContract(config: OrchestratorConfig | undefined, appName = "Pi Workspace"): string {
  ...
  return [
    ...
    `You are the Lead Architect and Orchestrator. Implementation and code changes are delegated to Herdr-managed Pi worker agents (worker model: ${active.workerModel}, thinking: ${active.workerThinking} (profile default; per-run effort is chosen by the adaptive rule below), max parallel workers: ${active.maxParallelWorkers}, strategy: ${active.delegationStrategy}).`,
