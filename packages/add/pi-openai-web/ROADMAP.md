...
One product: an always-on **OpenAI Web Lead Architect** running as a native Pi provider, a **strict bounded lead-tool allowlist** on MCP, and **asynchronous native Herdr execution** (`run | status | correct | stop`) over **Herdr-managed Pi workers only**.
...
- [x] `herdr` tool actions: `run` (validated 1–4 worker contract, explicit TUI confirmation, immediate handle), `status` (persisted lifecycle), `correct` (exact worker pane reuse with fresh turn evidence), `stop` (run or all owned panes).
